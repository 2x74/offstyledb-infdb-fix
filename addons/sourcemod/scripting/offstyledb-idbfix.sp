...
void GetStyleMapping(bool forceRefresh = false)
{
    ...
    if (!forceRefresh)
    {
        DebugPrint("[OSdb] Sending style mapping request");
    }
    else
    {
        DebugPrint("[OSdb] Force refresh requested");
    }
...
void SendRecord(char[] sMap, char[] sSteamID, char[] sName, int sDate, float time, float sync, int strafes, int jumps, int style, bool isWR, const char[] replayPath = "")
{
    ...
    // FIX: Always write replayPath to the DataPack so OnRecordSubmitted
    // can always safely ReadString, even when replayPath is an empty string.
